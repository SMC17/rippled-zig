...
test "known XRPL address vector" {
    const allocator = std.testing.allocator;
    const account_id = [_]u8{
    ...
    const address = try Base58.encodeAccountID(allocator, account_id);
    defer allocator.free(address);
    try std.testing.expectEqualStrings("rPickFLAKK7YkMwKvhSEN1yJAtfnB6qRJc", address);

    const decoded = try Base58.decodeAccountID(allocator, "rPickFLAKK7YkMwKvhSEN1yJAtfnB6qRJc");
    try std.testing.expectEqualSlices(u8, &account_id, &decoded);
}
